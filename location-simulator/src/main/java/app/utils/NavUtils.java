...
public class NavUtils {
     ...
     * @return
     */
    public static Point getPosition(Point pt1, double dist, double brg) {
        if (Double.doubleToRawLongBits(dist) == 0) {
            return pt1;
        }
        ...
        double lat2 = Math.asin(Math.sin(lat1) * Math.cos(dist / EARTH_RADIUS_IN_METERS) + Math.cos(lat1) * Math.sin(dist / EARTH_RADIUS_IN_METERS) * Math.cos(brgAsRadians));
        double x = Math.sin(brgAsRadians) * Math.sin(dist / EARTH_RADIUS_IN_METERS) * Math.cos(lat1);
        double y = Math.cos(dist / EARTH_RADIUS_IN_METERS) - Math.sin(lat1) * Math.sin(lat2);
